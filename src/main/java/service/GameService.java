...
public class GameService {
    ...
    public void start(Game game) {
        ...
        while (true) {
            ...
            if (inputRow.equalsIgnoreCase("esc")) { saveBoard(board, player, ai); continue; }

            String inputCol = console.readString("Oszlop: ");
            if (inputCol.equalsIgnoreCase("esc")) { saveBoard(board, player, ai); continue; }
    ...
    private void saveBoard(Board board, Player player, Player ai){
        try(FileWriter writer = new FileWriter("amoba_save.txt")){
            ...
            writer.write("  ");
            for(int c=0;c<board.getCols();c++) writer.write((char)('A'+c)+" ");
            writer.write("\n");
            for(int r=0;r<board.getRows();r++){
                writer.write((r+1)+" ");
